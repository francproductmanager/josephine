...
- Vocal-tone emotion (Google Gemini, not OpenAI): `gemini-3.6-flash` (override via `GEMINI_MODEL`) in `src/services/emotion-service.js`. Gemini is used because it accepts WhatsApp's ogg/opus inline; OpenAI's audio-input chat models take only wav/mp3 and transcoding would need a dependency. The model chooses the wording itself (open vocabulary): 1-3 words on tone/delivery/energy, answered directly in the user's language with one emoji (e.g. `😤 tense, rushed`), spliced into the localized `emotionIntro` template; a NEUTRAL verdict suppresses the line. Model output is gated by `sanitizeEmotion` (single line, ≤40 chars, ≤5 words, no digits/URLs/markup) since it lands verbatim in the reply. Fail-open: no `GEMINI_API_KEY`, any error, or a gated/NEUTRAL verdict → no mood line, transcription unaffected. Order: mood sentence, summary (long notes), transcription.
