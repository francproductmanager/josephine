...
- Vocal-tone emotion (Google Gemini, not OpenAI): the **Interactions API** (`POST /v1beta/interactions`, `gemini-3.8-flash`, override via `GEMINI_MODEL`) in `src/services/emotion-service.js`; generateContent is Google's legacy surface (a retired-model 404 on it broke the feature on day one). Gemini is used because it accepts WhatsApp's ogg/opus inline; OpenAI's audio-input chat models take only wav/mp3 and transcoding would need a dependency. Output is schema-enforced JSON `{neutral, description, confidence}`: the model authors a short open-vocabulary phrase (tone + delivery + energy, may describe evolution) directly in the user's language with one emoji; `sanitizeDescription` still gates it (single line, ≤140 chars, no digits/URLs/markup, em/en dashes converted to commas per house style: no em dashes in user-facing copy, ever). The reply opens with the tone block before summary and transcription: the localized `emotionIntro` sentence, plus a hedge line ONLY when the model's self-reported confidence is middling ("not fully sure" at 40-59, "moderately confident" at 60-79; 80+ shows nothing, the bands are ours since Google documents no calibration and observed values cluster at 85). Suppressed entirely when neutral, confidence < 40, gated, or on any error (fail-open; no `GEMINI_API_KEY` = feature off). Timing: the verdict runs in parallel from download and the reply waits for it only until transcription+summary are done plus a 4s grace (`EMOTION_GRACE_MS` in the pipeline; the fetch itself has a 20s inner cap so nothing dangles). The raw confidence still flows to the API response and logs.
...
- Machine-drafted translations for bg/cs/da/et/fi/lv/lt/mt/pt/sk/sl/zh/hi/ja await native-speaker review (likewise the `emotionIntro` key in every non-English language, added 2026-09-07)
